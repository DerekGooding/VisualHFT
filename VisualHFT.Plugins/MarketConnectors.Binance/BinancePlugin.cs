...
using MarketConnectors.Binance.ViewModels;
using System;
...
using ErrorEventArgs = VisualHFT.Commons.PluginManager.ErrorEventArgs;
...
public class BinancePlugin : BasePluginDataRetriever
{
    ...
    private Dictionary<string, OrderBook> _localOrderBooks = [];
    private Dictionary<string, BlockingCollection<IBinanceEventOrderBook>> _eventBuffers = [];
    private Dictionary<string, BlockingCollection<IBinanceTrade>> _tradesBuffers = [];

    private Dictionary<string, CancellationTokenSource> _ctDeltas = [];
    private Dictionary<string, CancellationTokenSource> _ctTrades = [];

    private Dictionary<string, long> _localOrderBooks_LastUpdate = [];
    ...
    {
        RaiseOnError(new ErrorEventArgs() { IsCritical = false, PluginName = Name, Exception = new Exception("Connection lost.") });
        // Start the HandleConnectionLost task without awaiting it
        Task.Run(HandleConnectionLost);
    }
    ...
    private OrderBook ToOrderBookModel(BinanceOrderBook data)
    {
        ...
        {
            Symbol = GetNormalizedSymbol(data.Symbol),
            SymbolMultiplier = 2, //???????
            DecimalPlaces = 2, //?????????
            ProviderID = _settings.Provider.ProviderID,
    ...
    {
        PluginSettingsView view = new();
        PluginSettingsViewModel viewModel = new(CloseSettingWindow)
        {
        ...
        viewModel.UpdateSettingsFromUI = () =>
        {
            ...
            _settings.Provider = new Provider() { ProviderID = viewModel.ProviderId, ProviderName = viewModel.ProviderName };
